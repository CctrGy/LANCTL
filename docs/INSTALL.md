...
contiene los ejecutables onefile `LANCTL.exe`, `lanip.exe`,
`lanwire.exe`, `lanrack.exe`, `lanaccess.exe` y `lanmon.exe`
...
El ZIP portable contiene `LANCTL.exe`, `lanip.exe`, `lanwire.exe`,
`lanrack.exe`, `lanaccess.exe`, `lanmon.exe`,
