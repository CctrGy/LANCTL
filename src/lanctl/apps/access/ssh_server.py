    ...
    def _execute(self, user, command):
        try:
            ...
            event = errors.from_exception(
                error,
                origin="LANCTL.Access.SSH.Command",
                code="ACCESS.SSH.COMMAND_REJECTED",
                level=44 if isinstance(error, PermissionError) else 34,
                details={"username": user.username},
                print_output=False,
            )
            return 2, f"Error {event.error_id}: {event.message}"
