...
class LanctlCommandAdapter:
    ...
    def command(arguments: list[str]) -> list[str]:
        ...
        return [
            sys.executable,
            str(Path(__file__).resolve().parents[2] / "lanctl.py"),
            *arguments,
        ]
    ...
    def execute(self, user, command: str) -> tuple[int, str]:
        ...
        try:
            ...
            event = errors.from_exception(
                error,
                origin="LANCTL.Access.Remote.Command",
                code="ACCESS.REMOTE.COMMAND_TIMEOUT",
                level=46,
                details={"timeout": self.timeout},
                print_output=False,
            )
            return 124, f"Tiempo de ejecucion agotado. [{event.error_id}]"
    ...
    def _root(arguments: list[str]) -> tuple[int, str]:
        from lanctl.apps.access.root_control import enqueue, forced_view, root_status
        ...
        if action == "status" and len(arguments) <= 1:
            ...
            if len(arguments) == 2:
                ...
                from lanctl.apps.access.root_control import default_forced_view
