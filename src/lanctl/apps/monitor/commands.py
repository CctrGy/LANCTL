...
from lanctl.apps.monitor.database import IncidentRepository as SqlIncidentRepository
from lanctl.apps.monitor.database import MetricsStore, MonitorDatabase
from lanctl.apps.monitor.database import SessionRepository as SqlSessionRepository
from lanctl.apps.monitor.incidents import IncidentManager
from lanctl.apps.monitor.lifecycle import SingletonLock
from lanctl.apps.monitor.models import MonitorTargetPlan
from lanctl.apps.monitor.operations import (
    BoundedRunner,
    identify_target,
    ping_targets,
    scan_target,
)
from lanctl.apps.monitor.reports import ReportBuilder
from lanctl.apps.monitor.service import MonitorService
from lanctl.apps.monitor.sessions import SessionManager
from lanctl.core.conditions import duration
from lanctl.core.config import load_config, save_config
...
def _monitor_once(args, sessions, config):
    ...
    from lanctl.apps.monitor.checks import availability
...
def _service_host_action(args, sessions):
    ...
    from lanctl.infrastructure.platform.windows_service import WindowsServiceHost
...
def _smb_scan(args, config):
    ...
    from lanctl.core.plugins.smb_runtime import SMBService
...
def _restart_action(args, sessions):
    ...
    if lock.status().get("running"):
        ...
        event = errors.emit(
            origin="LANCTL.Monitor.Restart.Stop",
            code="MONITOR.RESTART.STOP_TIMEOUT",
            level=46,
            message="El proceso monitor no se detuvo dentro del límite",
            details={"pid": old_pid},
            print_output=False,
        )
        return {
...
def _platform():
    ...
    from lanctl.infrastructure.platform.base import PlatformAdapter
...
def _lanctl_command(*arguments):
    ...
    if words and words[0] == "monitor":
        words.pop(0)
    if getattr(sys, "frozen", False):
        launcher = Path(sys.executable).with_name("lanmon.exe" if os.name == "nt" else "lanmon")
        if not launcher.is_file():
            raise FileNotFoundError("falta el launcher LANMON junto al ejecutable")
        return [str(launcher), *words]
    return [
        sys.executable,
        "-c",
        "import sys;sys.path.insert(0,"
        + repr(str(Path(__file__).resolve().parents[3]))
        + ");from lanctl.bootstrap.lanmon import main;raise SystemExit(main())",
        *words,
    ]
