...
PRESENCE = {"unknown", "online", "offline", "waking"}
HEALTH = {"healthy", "warning", "critical", "maintenance"}
SESSION_STATES = {"pending", "active", "stopping", "completed", "expired", "cancelled", "error"}
AUTHORITIES = {"observe", "operate", "administer"}
MODES = {"permanent", "temporary", "diagnostic", "once"}
...
class MonitorProfile:
    ...
    presence_interval: float = 60
    critical_interval: float = 15
    discovery_interval: float = 300
    services_interval: float = 1800
    deep_interval: float = 86400
    timeout: float = 0.8
    workers: int = 8
    jitter: float = 0.1
...
@dataclass(frozen=True)
class MonitorTargetPlan:
    """Plan resuelto para un dispositivo durante una sesión de monitorización."""
...
class MonitorSession:
    ...
    def __post_init__(self):
        if (
            ...
            or self.authority not in AUTHORITIES
            or self.status not in SESSION_STATES
        ):
            raise ValueError("sesión monitor no válida")
