...
def multicast_discover(methods: tuple[str, ...], timeout: float) -> dict[str, set[str]]:
    ...
    try:
        ...
        errors.from_exception(
            error,
            origin="LANCTL.Discovery.Multicast",
            code="DISCOVERY.MULTICAST.PARTIAL",
            level=14,
            details={"methods": list(methods), "findings": len(findings)},
            print_output=False,
        )
        return findings
    finally:
...
def discover_services(methods: tuple[str, ...], timeout: float) -> dict[str, set[str]]:
    ...
    from lanctl.core.plugins.scanners import run_scanner_extensions
