...
from lanctl.apps.ip.infrastructure.services.network_discovery import discover_services
...
def local_ipv4() -> ipaddress.IPv4Address:
    ...
    try:
        ...
        errors.from_exception(
            error,
            origin="LANCTL.Scanner.LocalIPv4.Route",
            code="SCANNER.LOCAL_IPV4.HOSTNAME_FALLBACK",
            level=14,
            print_output=False,
            once_key="scanner.local-ipv4.hostname-fallback",
        )
        try:
...
class LanScanner:
    ...
    def _retry_icmp(self, host_strings, attempts, active_ips, progress):
        ...
        if attempts > 1:
            ...
            errors.emit(
                level=6,
                origin="LANCTL.Scanner.ICMP.Retry",
                code="SCANNER.ICMP.RETRY_SUMMARY",
                message="Se completaron rondas adicionales de detección ICMP",
                details={
                    "attempts": attempts,
                    "recovered": len(active_ips) - before,
                    "targets": len(host_strings),
                },
                print_output=False,
            )
