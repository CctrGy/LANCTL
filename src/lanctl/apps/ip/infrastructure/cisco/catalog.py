...
from lanctl.apps.ip.infrastructure.cisco.models import CommandSpec, Risk


def _show(path: str, help_text: str) -> CommandSpec:
    parts = tuple(path.split())
    return CommandSpec(
        id="switch." + ".".join(parts),
        path=parts,
        aliases=(),
        context=("switch",),
        risk=Risk.READ_ONLY,
...
PORT_COMMANDS = (
    CommandSpec(
        "switch.port.show.status",
        ("port", "show", "status"),
        (),
        ("switch", "port"),
        Risk.READ_ONLY,
        "Estado del puerto.",
        ("show interfaces status {native_port}",),
        True,
    ),
    CommandSpec(
        "switch.port.show.description",
        ("port", "show", "description"),
        (),
        ("switch", "port"),
        Risk.READ_ONLY,
        "Descripción del puerto.",
        ("show interfaces description {native_port}",),
        True,
    ),
    CommandSpec(
        "switch.port.show.config",
        ("port", "show", "config"),
        (("port", "show", "configuration"),),
        ("switch", "port"),
        Risk.READ_ONLY,
        "Configuración del puerto.",
        ("show interfaces configuration {native_port}",),
        True,
    ),
    CommandSpec(
        "switch.port.show.errors",
        ("port", "show", "errors"),
        (),
        ("switch", "port"),
        Risk.READ_ONLY,
        "Errores del puerto.",
        ("show interfaces errors {native_port}",),
        True,
    ),
    CommandSpec(
        "switch.port.show.vlan",
        ("port", "show", "vlan"),
        (),
        ("switch", "port"),
        Risk.READ_ONLY,
        "VLAN del puerto.",
        ("show interfaces switchport {native_port}",),
        True,
    ),
    CommandSpec(
        "switch.port.set.description",
        ("port", "set", "description"),
        (),
        ("switch", "port"),
        Risk.CONFIG_CHANGE,
        "Cambia la descripción.",
        ("configure terminal", "interface {native_port}", "description {value}"),
        True,
        "description",
    ),
    CommandSpec(
        "switch.port.set.speed",
        ("port", "set", "speed"),
        (),
        ("switch", "port"),
        Risk.CONFIG_CHANGE,
        "Cambia la velocidad.",
        ("configure terminal", "interface {native_port}", "speed {value}"),
        True,
        "speed",
    ),
    CommandSpec(
        "switch.port.set.duplex",
        ("port", "set", "duplex"),
        (),
        ("switch", "port"),
        Risk.CONFIG_CHANGE,
        "Cambia el dúplex.",
        ("configure terminal", "interface {native_port}", "duplex {value}"),
        True,
        "duplex",
    ),
    CommandSpec(
        ...
        Risk.CONFIG_CHANGE,
        "Habilita el puerto.",
        ("configure terminal", "interface {native_port}", "no shutdown"),
        True,
    ),
    CommandSpec(
        "switch.port.disable",
        ...
        ("configure terminal", "interface {native_port}", "shutdown"),
        True,
    ),
    CommandSpec(
        "switch.port.reset",
        ...
        Risk.DISRUPTIVE,
        "Restaura el puerto.",
        ("configure terminal", "default interface {native_port}"),
        True,
    ),
    CommandSpec(
        "switch.save-config",
