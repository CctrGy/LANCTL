...
from lanctl.apps.ip.infrastructure.cisco.catalog import find_spec
from lanctl.apps.ip.infrastructure.cisco.models import (
    CommandPlan,
    CommandSpec,
    PortProfile,
    SwitchProfile,
...
class CiscoPlanner:
    ...
    def _validate(spec: CommandSpec, value: str) -> str:
        ...
        if spec.argument == "speed" and value.casefold() not in {
            "auto",
            "10",
            "100",
            "1000",
    ...
    def plan(self, tokens: list[str], selected_port: PortProfile | None = None) -> CommandPlan:
        ...
        if selected_port and (
            (lowered[0] == "show" and len(tokens) == 2)
            or lowered[0] in {"set", "enable", "disable"}
        ):
            tokens = ["port", *tokens]
        lowered = [token.casefold() for token in tokens]
        ...
        return CommandPlan(
            ...
            spec.risk,
            commands,
            port.id if port else "",
            port.native if port else "",
