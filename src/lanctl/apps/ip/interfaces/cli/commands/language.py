...
from lanctl.shared.i18n import get_language_manager, t


def register_language_command(commands: argparse._SubParsersAction) -> None:
    command = commands.add_parser(
        "language", aliases=["languages", "lang"], help=t("LANCTL.LANGUAGE.COMMAND.HELP")
    )
    actions = command.add_subparsers(dest="language_action", metavar="ACTION", required=True)
    actions.add_parser("list", help=t("LANCTL.LANGUAGE.ACTION.LIST")).set_defaults(
        language_handler=_list
    )
    ...
    info.add_argument(
        "language", nargs="?", help="Language code or name; active language by default."
    )
...
def _list(args) -> int:
    ...
    print(
        f"{Style.BRIGHT}{Fore.CYAN}{t('LANCTL.LANGUAGE.FIELD.CODE'):<10} {t('LANCTL.LANGUAGE.FIELD.LANGUAGE'):<20} {t('LANCTL.LANGUAGE.FIELD.REGION'):<18} {t('LANCTL.LANGUAGE.FIELD.COVERAGE'):>10} {t('LANCTL.LANGUAGE.FIELD.ACTIVE'):>8}{Style.RESET_ALL}"
    )
    print("-" * 72)
    for catalog in manager.list():
        result = manager.validate(catalog.path) if catalog.path else {"coverage": 100.0}
        print(
            f"{catalog.code:<10} {catalog.native_name:<20} {catalog.region or '-':<18} {result['coverage']:>9.1f}% {'O' if catalog.code == manager.selected else '-':>8}"
        )
    return 0
...
def _use(args) -> int:
    ...
    update_config(lambda config: config.update(language=catalog.code))
...
def _info(args) -> int:
    ...
    result = (
        manager.validate(catalog.path)
        if catalog.path
        else {"translated": len(catalog.strings), "total": len(catalog.strings), "coverage": 100.0}
    )
    print(
        json.dumps(
            {
                "code": catalog.code,
                "name": catalog.name,
                "nativeName": catalog.native_name,
                "region": catalog.region,
                "version": catalog.version,
                "author": catalog.author,
                "owner": catalog.owner,
                "active": catalog.code == manager.selected,
...
def _validate(args) -> int:
    ...
    ok(
        t("LANCTL.LANGUAGE.STATUS.VALID"),
        f"{result['catalog'].native_name} | {result['coverage']:.1f}%",
    )
    return 0
