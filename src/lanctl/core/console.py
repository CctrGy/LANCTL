...
from lanctl.core.layout import terminal_columns, wrapped_lines
...
def status(label: str, message: str, kind: str = "ok", *, log: bool = True) -> None:
...
def error(message: str, *, log: bool = True) -> None:
    status(t("LANCTL.COMMON.STATUS.ERROR"), message, "error", log=log)
