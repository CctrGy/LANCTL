...
void UsbConsole::printBanner() {
  Serial.println();
  Serial.println("RackMonitor Hardware V1 - USB CLI");
  Serial.println("Type 'help' for commands.");
  Serial.print("rackmonitor# ");
}
...
void UsbConsole::executeLine() {
  ...
  if (normalized == "exit" || normalized == "logout" || normalized == "quit") {
    Serial.println("USB CLI session closed. Press Enter to reopen.");
...
void UsbConsole::update() {
  while (Serial.available()) {
    ...
    if (c == '\r' || c == '\n') {
      ...
      Serial.print("\b \b");
    } else if (c >= 32 && c < 127 && line_.length() < 255) {
      line_ += c;
      Serial.print(c);
    }
  }
