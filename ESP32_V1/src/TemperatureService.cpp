...
void TemperatureService::begin() {
  topSensors_.begin();
  bottomSensors_.begin();
  topSensors_.setResolution(11);
  bottomSensors_.setResolution(11);
  topSensors_.setWaitForConversion(false);
  bottomSensors_.setWaitForConversion(false);
  topSensors_.requestTemperatures();
  bottomSensors_.requestTemperatures();
  lastRead_ = millis();
}

void TemperatureService::readGroup(DallasTemperature &bus, TemperatureGroup &group, float mismatchLimit) {
  group = TemperatureGroup{};
  const uint8_t count = min<uint8_t>(bus.getDeviceCount(), 2);
  for (uint8_t i = 0; i < count; ++i) {
    ...
    probe.valid = probe.celsius != DEVICE_DISCONNECTED_C && isfinite(probe.celsius) &&
                  probe.celsius >= -55.0F && probe.celsius <= 125.0F;
    if (probe.valid) {
      group.mean = (group.validCount == 0) ? probe.celsius : group.mean + probe.celsius;
      ++group.validCount;
    }
  }
  if (group.validCount) group.mean /= group.validCount;
  group.degraded = group.validCount == 1;
  group.mismatch = group.validCount == 2 &&
...
void TemperatureService::update(float mismatchLimit) {
  ...
  snapshot_.deltaT = (snapshot_.top.validCount && snapshot_.bottom.validCount)
                         ? snapshot_.top.mean - snapshot_.bottom.mean : NAN;
  topSensors_.requestTemperatures();
  bottomSensors_.requestTemperatures();
  lastRead_ = millis();
}
...
String TemperatureService::describe() const {
  auto groupText = [](const char *name, const TemperatureGroup &g) {
    ...
    if (g.degraded) s += ", DEGRADED";
    if (g.mismatch) s += ", MISMATCH";
    return s;
  };
  String out = groupText("TOP", snapshot_.top) + "\r\n" + groupText("BOTTOM", snapshot_.bottom);
