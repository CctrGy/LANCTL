...
class MonitorTests(unittest.TestCase):
    def test_scheduler_uses_monotonic_queue_no_overlap_and_backoff(self):
        now = [0.0]
        calls = []
        pool = MonitorWorkerPool(1, 1, ImmediateExecutor())
        scheduler = MonitorScheduler(pool, lambda: now[0], jitter=0)
        scheduler.schedule(
            "task", "dev", "availability", 10, 1, lambda target, timeout: calls.append(target)
        )
    ...
    def test_worker_backpressure_is_bounded(self):
        pool = MonitorWorkerPool(1, 1, MagicMock())
        pool.executor.submit.return_value = MagicMock()
        self.assertIsNotNone(pool.submit(lambda: None))
    ...
    def test_bounded_runner_limits_and_cooperative_cancellation(self):
        ...
        rows = runner.run(
            lambda: calls.append(1), interval=2, duration=10, cancel=lambda: len(calls) >= 3
        )
        self.assertEqual(len(rows), 3)
        with self.assertRaises(ValueError):
            runner.run(lambda: None, interval=0.1, duration=1)
    ...
    def test_state_thresholds_and_recovery_are_hysteretic(self):
        ...
        for _ in range(2):
            state, changed, _ = evaluator.evaluate(CheckResult("availability", "dev", False))
        self.assertEqual(state.presence, "unknown")
        state, changed, _ = evaluator.evaluate(CheckResult("availability", "dev", False))
        self.assertEqual(state.presence, "offline")
        self.assertTrue(changed)
        evaluator.evaluate(CheckResult("availability", "dev", True))
        self.assertEqual(state.presence, "offline")
        evaluator.evaluate(CheckResult("availability", "dev", True))
        self.assertEqual(state.presence, "online")

    def test_incidents_dedupe_acknowledge_and_resolve(self):
        manager = IncidentManager(InMemoryIncidentRepository())
        first = manager.open("dev", "critical", "offline", "monitor.state.evaluate")
        second = manager.open("dev", "critical", "offline", "monitor.state.evaluate")
        self.assertEqual(first.incidentId, second.incidentId)
        manager.acknowledge(first.incidentId)
        self.assertEqual(manager.resolve("dev", "offline").status, "resolved")

    def test_sessions_expire_without_authority_escalation(self):
        now = [datetime(2026, 8, 3, 10, 0).astimezone()]
        manager = SessionManager(InMemorySessionRepository(), clock=lambda: now[0])
        session = manager.start(
            "mgr", "project", mode="diagnostic", authority="observe", duration=60
        )
        self.assertEqual(session.authority, "observe")
        now[0] += timedelta(seconds=61)
        self.assertEqual(manager.expire().status, "expired")

    def test_network_identity_mismatch_requires_confirmation(self):
        resolver = IdentityResolver(
            SimpleNamespace(identity=lambda: NetworkIdentity(gatewayMac="AA"))
        )
        with self.assertRaises(PermissionError):
            resolver.validate(NetworkIdentity(gatewayMac="BB"), resolver.resolve())
        self.assertFalse(
            resolver.validate(NetworkIdentity(gatewayMac="BB"), resolver.resolve(), confirm=True)
        )

    def test_check_registry_validates_plugin_limits(self):
        registry = CheckRegistry()
        with self.assertRaises(ValueError):
            registry.register(CheckSpec("bad", "plugin", lambda: None, 1, 1))
        registry.register(CheckSpec("good", "plugin", lambda: None, 5, 1))
        self.assertEqual(registry.get("good").owner, "plugin")

    def test_singleton_recovers_stale_lock(self):
        with tempfile.TemporaryDirectory() as temporary:
            path = Path(temporary) / "monitor.lock"
            path.write_text('{"pid":99999999}')
            lock = SingletonLock(path).acquire()
            self.assertTrue(lock.status()["running"])
            lock.release()
            self.assertFalse(path.exists())

    def test_systemd_unit_is_safe_and_install_needs_confirmation(self):
        adapter = LinuxPlatform()
        unit = adapter.unit_text("/usr/bin/python3", "/tmp/test.vlf")
        self.assertIn(
            'ExecStart="/usr/bin/python3" -m app monitor foreground --project "/tmp/test.vlf"', unit
        )
        self.assertIn("NoNewPrivileges=true", unit)
        self.assertEqual(adapter.service("install", confirm=False).status, "blocked")
        with self.assertRaises(ValueError):
            adapter.unit_text("bad\ncommand", "/tmp/test.vlf")
        with self.assertRaises(ValueError):
            adapter.unit_text("/usr/bin/python3", "")
    ...
    def test_monitor_database_is_closed_when_an_action_fails(self):
        args = build_parser().parse_args(["monitor", "invalid-action"])
        context = MagicMock()
    ...
    def _cli_context(self, temporary):
        ...
        DeviceDatabase(devices).upsert(
            [{"IP": "192.168.1.8", "MAC": "02:11:22:33:44:55", "ALIAS": "NAS"}]
        )
        return {
            "database": str(devices),
            "monitorDatabase": str(root / "monitor.db"),
            "monitorProfiles": str(root / "profiles.json"),
            "monitorAssignments": str(root / "assignments.json"),
            "monitorRuntime": str(root / "sessions.json"),
            "monitorIncidents": str(root / "incidents.json"),
            "monitorLock": str(root / "lock"),
            "timeout": 0.1,
            "workers": 1,
            "activeProject": None,
        }
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                with contextlib.redirect_stdout(StringIO()):
                    self.assertEqual(args.handler(args), 0)
            self.assertEqual(runner.call_args.kwargs["interval"], 2)
            self.assertEqual(runner.call_args.kwargs["duration"], 10)
    ...
    def test_scan_presence_and_identify_routes_are_real(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            identify = build_parser().parse_args(
                ["monitor", "identify", "NAS", "--monitor-db", config["monitorDatabase"]]
            )
            import contextlib
            from io import StringIO
            ...
            with (
                ...
                self.assertEqual(scan.handler(scan), 0)
                self.assertEqual(identify.handler(identify), 0)
    ...
    def test_health_and_events_read_real_repositories(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            health = build_parser().parse_args(
                ["monitor", "health", "--monitor-db", config["monitorDatabase"]]
            )
            events = build_parser().parse_args(
                ["monitor", "events", "--monitor-db", config["monitorDatabase"]]
            )
            history = MagicMock()
            ...
            with (
                ...
                self.assertEqual(events.handler(events), 0)
    ...
    def test_restart_never_kills_without_verified_active_lock(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(json.loads(output.getvalue())["status"], "not-running")
