...
class WolTests(unittest.TestCase):
    ...
    def test_packet_has_standard_layout_and_rejects_unsafe_macs(self):
        ...
        for value in ("00:00:00:00:00:00", "FF:FF:FF:FF:FF:FF", "01:00:00:00:00:01", "bad"):
            with self.subTest(value=value), self.assertRaises(ValueError): validate_mac(value)

    def test_sender_is_injectable_and_never_uses_shell(self):
        sock = MagicMock(); factory = MagicMock(return_value=sock)
    ...
    def test_closed_condition_parser_and_combinators(self):
        context = ConditionContext(online=lambda name: name == "up", target="down", now=datetime(2026, 8, 3, 8, 0).astimezone())
        ...
        with self.assertRaises(ValueError): parse_condition("__import__('os').system('x')")
    ...
    def test_dry_run_resolves_device_without_sending(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch("app.commands.wol._online", return_value=False), patch("app.commands.wol.send_magic_packet") as send, patch("app.commands.wol.write_log"):
                ...
                import contextlib
                output=StringIO()
                with contextlib.redirect_stdout(output): code=run_wol(args)
            self.assertEqual(code,0); send.assert_not_called(); self.assertEqual(json.loads(output.getvalue())["status"],"sent")
    ...
    def test_remote_power_is_blocked_without_transport(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch("app.commands.wol.write_log"):
                ...
                output=StringIO()
                with contextlib.redirect_stdout(output): code=run_wol(args)
            payload=json.loads(output.getvalue()); self.assertEqual(code,1); self.assertEqual(payload["status"],"blocked"); self.assertEqual(payload["error"]["code"],"WOL.POWER.UNSUPPORTED")
    ...
    def test_false_condition_skips_without_sending(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch("app.commands.wol._online", return_value=False), patch("app.commands.wol.send_magic_packet") as send, patch("app.commands.wol.write_log"):
                ...
                output=StringIO()
                with contextlib.redirect_stdout(output): code=run_wol(args)
            self.assertEqual(code,0); self.assertEqual(json.loads(output.getvalue())["status"],"skipped"); send.assert_not_called()
