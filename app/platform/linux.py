    ...
    def service(self, action, **kwargs):
        if action == "status":
            completed = subprocess.run(
                ["systemctl", "is-active", "lanctl-monitor.service"],
                capture_output=True,
                text=True,
                check=False,
            )
        ...
        if action in {"start", "stop", "restart"}:
            completed = subprocess.run(
                ["systemctl", action, "lanctl-monitor.service"],
                capture_output=True,
                text=True,
                check=False,
            )
        ...
        if action in {"install", "uninstall"}:
            if not kwargs.get("confirm"):
                return ServiceResult(
                    True, "blocked", "La instalación del servicio requiere confirmación explícita"
                )
            if os.geteuid() != 0:
                return ServiceResult(True, "blocked", "Se requieren privilegios de administrador")
            unit = Path("/etc/systemd/system/lanctl-monitor.service")
            if action == "install":
                unit.write_text(
                    self.unit_text(kwargs["executable"], kwargs["project"]), encoding="utf-8"
                )
            elif unit.exists():
                unit.unlink()
