...
def register_protocol_command(commands: argparse._SubParsersAction) -> None:
    ...
    command.add_argument("action", choices=("show", "configure"), help="Consulta o modifica el protocolo.")
    ...
    command.add_argument("--host-key", action="append", default=[], help="Algoritmo de clave de host permitido; se puede repetir.")
    command.add_argument("--kex", action="append", default=[], help="Algoritmo de intercambio de claves; se puede repetir.")
...
def run_protocol(args: argparse.Namespace) -> int:
    ...
    options = dict(SSH_PROFILES[args.profile]) if args.profile else {
        "port": args.port,
        "driver": args.driver,
