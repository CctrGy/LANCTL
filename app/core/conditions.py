...
_DURATION = re.compile(r"^(\d+)(s|m|h|d)$", re.I)
...
def duration(value: str) -> float:
    ...
    amount = int(match.group(1)); unit = match.group(2).casefold()
...
class ConditionContext:
    def __init__(self, *, online: Callable[[str], bool], target: str,
                 last_seen: str = "", group_members: Callable[[str], list[str]] | None = None,
...
def evaluate(condition: Condition, context: ConditionContext) -> bool:
    ...
    if kind == "state": return context.online(context.target) == (values[0].casefold() == "online")
    if kind == "probe": return context.online(context.target)
    if kind == "port": return context.port_open(context.target, int(values[0]))
    if kind == "offline-for":
        if context.online(context.target): return False
        return _age(context) >= duration(values[0])
    if kind == "last-seen": return _age(context) > duration(values[0])
    if kind == "device": return context.online(values[0]) == (values[1].casefold() == "online")
    if kind == "group":
        states = [context.online(item) for item in context.group_members(values[0])]
        return bool(states) and (all(states) if values[1].casefold() == "all-online" else any(not x for x in states))
    if kind == "task":
        actual = context.task_state(values[0]); expected = values[1].casefold()
        return actual == expected or expected == "completed" and actual in {"success", "failed", "error", "timeout"}
    if kind in {"smb-state", "smb-resource"}:
        target = values[1] if kind == "smb-state" else values[1]
        observed = context.smb_observation(target)
        if not observed or not observed.get("observedAt"): return False
        if kind == "smb-state":
            state = observed.get("state")
            return state == ("authentication-required" if values[0].casefold()=="authentication.required" else "available") if values[0].casefold()!="unavailable" else state != "available"
        shares = observed.get("smb",{}).get("shares",[]); resource=values[2].casefold(); action=values[0].casefold()
        matches=[item for item in shares if str(item.get("name","")).casefold()==resource]
        if action=="printer.exists": return any(item.get("type")=="printer" for item in matches)
        if action=="share.exists": return bool(matches)
    ...
    if kind in {"time", "between"}:
        ...
        return current > values[1] if values[0].casefold() == "after" else current < values[1] if kind == "time" else values[0] <= current <= values[1]
    if kind == "weekday":
        names = ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"]
        aliases = {"lunes":"monday","martes":"tuesday","miércoles":"wednesday","miercoles":"wednesday","jueves":"thursday","viernes":"friday","sábado":"saturday","sabado":"saturday","domingo":"sunday"}
        spec = aliases.get(values[0].casefold(), values[0].casefold())
        current = names[context.now.weekday()]
        if spec == "monday-friday": return context.now.weekday() < 5
        return current == spec
    return False


def _age(context: ConditionContext) -> float:
    if not context.last_seen: return float("inf")
    seen = datetime.fromisoformat(context.last_seen)
    if seen.tzinfo is None: seen = seen.replace(tzinfo=context.now.tzinfo)
